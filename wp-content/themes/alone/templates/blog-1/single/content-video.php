...
<article id="post-<?php the_ID(); ?>" <?php post_class( "post post-details" ); ?> itemscope="itemscope" itemtype="http://schema.org/BlogPosting" itemprop="blogPost">
	<div class="fw-col-inner">
        ...
        <!-- Post video -->
    		<?php if(!empty($alone_post_options['video'])) { ?>
    			<div class="post-block-video-wrap" style="background: url(<?php echo esc_attr($alone_post_thumbnail_url); ?>) no-repeat center center / cover;">
    				<?php echo "{$alone_post_options['video']}"; ?>
    			</div>
    		<?php } ?>
