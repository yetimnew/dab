<?php
class WPBakeryShortCode_bt_portfolio_carousel extends WPBakeryShortCode {
	...
	protected function content( $atts, $content = null ) {
		...
		if (isset($category) && $category != '') {
			...
			$args['tax_query'] = array(
				array(
					'taxonomy' => 'fw-portfolio_category',
					'field' => 'slug',
					'terms' => $taxonomy
				)
			);
		...
		$custom_script = "jQuery(document).ready(function($) {
							setTimeout( function() {
								$('.bt-portfolio-carousel-element .owl-carousel').each(function() {
					...
					<?php
						if($rows == 1){
							while ( $wp_query->have_posts() ) { $wp_query->the_post();
								require get_template_directory().'/framework-customizations/extensions/custom-js-composer/vc_layout/portfolio-'.$layout.'.php';
							}
						}else{
...
vc_map(array(
	'name' => esc_html__('Portfolio Carousel', 'alone'),
	'base' => 'bt_portfolio_carousel',
