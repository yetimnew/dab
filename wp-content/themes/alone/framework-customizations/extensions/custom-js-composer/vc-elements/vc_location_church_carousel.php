...
<div <?php echo esc_attr($attr_id); ?> class="<?php echo esc_attr($css_class); ?>">
  <div class="vc-custom-inner-wrap">
	...
	if(count($posts_data) > 0):
		...
		$time_loca = get_post_meta($item['post_id'], '_ctc_location_times', true);
		$temp_variables = array(
			// '{image_html}'    => '',
			'{pid}'						=> $item['post_id'],
			'{readmore_html}' => '<a href="'. $item['post_link'] .'" class="location-view-detail">'. __('Location Page', 'alone') .' <span class="ion-ios-arrow-thin-right"></span></a>',
